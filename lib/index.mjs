...
export const plot = (name, entries, markings, color = [0, 50, 200]) => {
    ...
    for (const marking of markings) {
        ...
        const start_y = work_height - height_needed + 14;
        ctx.font = "14px Arial";
        ctx.fillStyle = `rgb(50, 50, 50)`;
        ctx.fillText(marking[1], 1, start_y);
    }
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo((w - total_needed) / 2, work_height + 16);
    ctx.lineTo(x_offset, work_height + 16);
    ctx.stroke();
...
export const graph = (name, entries, markings) => {
    ...
    for (const graph of entries) {
        ...
        if (points.length === 1) {
            const point = points[0] + top_offset;
            ctx.moveTo(offset, work_height - work_height * point);
            ctx.lineTo(point_len, work_height * point);
            ctx.stroke();
        } else {
            {
                ctx.moveTo(
