...
export const generateFileSavePath = () => {
  return new Promise((resolve, reject) => {
    let buffer = "";
    if(process.platform === "darwin") {
      const proc = spawn("osascript", ["-e", `tell application (path to frontmost application as text)
      ...
      proc.on("exit", (code) => {
        if(code === 0)
          ...
          reject(buffer.trim());
            ...
            const proc = spawn("powershell.exe", ["-encodedCommand", Buffer.from(`
...
if ($SaveFile -ne "") 
{
    ...
    exit 1
      ...
      const proc = spawn(join(import.meta.dirname, "gtk-saver", "gtk-saver"), []);
      proc.stdout.on("data", d => buffer += d.toString());
      proc.on("exit", (code) => {
        if(code === 0)
          resolve(buffer.trim());
...
export const easyWindow = (title, buffer, w, h, type = "rgba", winCb = null) => {
...
const addAllowSave = (window, canvas) => {
   window.setTextCallback((code) => {
      if(String.fromCharCode(code) === "S") {
          generateFileSavePath().then(res => {
              if(res.endsWith(".png")) {
                toPNG(res, canvas, `rgb(240, 240, 240)`);
              } else if (res.endsWith(".jpg") || res.endsWith(".jpeg")) {
                toJPEG(res, canvas);
              }
          }).catch(err => null)
      }
...
export const plot = (
  ...
  const row_width = Math.min((work_width / entries.length), 60) + options.spacing;
...
export const pie = (
  ...
  const centerX = 5 + (work_height * 0.45);
  const centerY = work_height / 2;

  const radius = work_height * 0.45;
  ...
  for (const v of entries) {
    ...
    if (Array.isArray(v) && typeof v[1] === "string") {
      ...
      ctx.font = "15px Arial";
      ctx.fillRect(centerX + radius + 5, textY, 10, 10);
      ctx.fillStyle = `rgb(50, 50, 50)`;
      ctx.fillText(name, centerX + radius + 20, textY + 11);
      textY += 22;
    }
    startAngle = endAngle;
  }
...
export const toWindow = async (name, o) => {
  const { canvas, w, h } = o;
  return easyWindowWithBounds(name, canvas.toBuffer("raw"), w, h, w, h, "bgra", (wind) => {
    addAllowSave(wind, o);
  });
};

export const toPNG = (path, o, background = "rgb(200, 200, 200)") => {
  let {canvas} = o;
  if(background) {
  ...
  const fout = fs.createWriteStream(path);
  stream.pipe(fout);
  return new Promise((resolve, reject) => {
    fout.on("error", err => reject(err));
    fout.on("finish", () => resolve());
  })
}

export const toJPEG = (path, o, quality = 0.95) => {
  const {canvas} = o;
  const stream = canvas.createJPEGStream({quality, chromaSubsampling: false});
  const fout = fs.createWriteStream(path);
  stream.pipe(fout);
  return new Promise((resolve, reject) => {
    fout.on("error", err => reject(err));
